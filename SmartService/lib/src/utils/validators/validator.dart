...
class TValidator {
  ...
  static Widget validatePasswordNew(String value) {
    if (value == null || value.isEmpty) {
      return TextCustom(TheText: "Le mot de passe est réquis.", TheTextSize: 12, TheTextMaxLines: 2,);
    }

    // Check for minimum password lenght
    if (value.length < 8) {
      return TextCustom(TheText: "Le mot de passe doit contenir au moins 08 caractères.", TheTextSize: 12, TheTextMaxLines: 2,);
    }

    // Check for uppercase letters
    if (!value.contains(RegExp(r'[A-Z]'))) {
      return TextCustom(TheText: "Le mot de passe doit contenir au moins une lettre majuscule.", TheTextSize: 12, TheTextMaxLines: 2,);
    }

    // Check for numbers
    if (!value.contains(RegExp(r'[0-9]'))) {
      return TextCustom(TheText: "Le mot de passe doit contenir au moins un chiffre.", TheTextSize: 12, TheTextMaxLines: 2,);
    }

    // Check for specials characters
    if (!value.contains(RegExp(r'[!@#$%^&*(),.?":{}|<>]'))) {
      return TextCustom(TheText: "Le mot de passe doit contenir au moins un caractère spécial.", TheTextSize: 12, TheTextMaxLines: 2,);
    }
